package com.helium.mixin.meteor;

import java.util.List;
import java.time.Instant;
import java.time.Duration;

import com.helium.util.HeliumUtils;
...
import com.helium.mixin.meteor.accessor.CategoryAccessor;
    ...
    private void cycleCategoryIcons(WContainer c, Category category, List<Module> moduleList, CallbackInfoReturnable<WWindow> cir) {
        if (category.name.equals("Stardust")) {
            if (!theme.categoryIcons()) {
                ((CategoryAccessor) category).setIcon(HeliumUtils.chooseMenuIcon());
