...
public class BlockUtilsWorld {
    ...
    public static BlockPos LowestDist(PlayerEntity a) {
        ...
        if (south1 < north1 && south1 < east1 && south1 < west1 && south1 <= 6 && mc.world.getBlockState(south).isReplaceable()) return south;
        if (east1 < north1 && east1 < south1 && east1 < west1 && south1 <= 6 && mc.world.getBlockState(east).isReplaceable()) return east;
        if (north1 < south1 && north1 < east1 && north1 < west1 && north1 <= 6 && mc.world.getBlockState(north).isReplaceable()) return north;
        if (west1 < north1 && west1 < south1 && west1 < east1 && west1 <= 6 && mc.world.getBlockState(west).isReplaceable()) return west;
        return null;
    }
    ...
    public static class SetBlockResult {
        ...
        public boolean S() {
            ...
            if (!block.getDefaultState().canPlaceAt(mc.world, pos)) {
                reset();
                return false;
            }
            ...
            BlockHitResult hitresult = new BlockHitResult(new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ()), direct, pos, true);
    ...
    public static void swap(int s) {
        if (s != ((PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot() && s >= 0 && s < 9) {
            mc.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(s));
            ((PlayerInventoryAccessor) mc.player.getInventory()).setSelectedSlot(s);
        }
    }
    ...
    public static void rotateBl(BlockPos bp) {
        ...
        mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(rotations[0], rotations[1], mc.player.isOnGround(), false));
    }
    ...
    public static void rotate(Vec3d vec, boolean sendPacket) {
        ...
        if (sendPacket) mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(rotations[0], rotations[1], mc.player.isOnGround(), false));
