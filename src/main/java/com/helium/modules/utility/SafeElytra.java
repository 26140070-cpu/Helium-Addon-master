...
public class SafeElytra extends Module {
    public SafeElytra() {
        super(HeliumAddon.CATEGORY, "Safe Elytra", "Attempts to prevent dying from kinetic energy by Donalp012.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (verticalVelocity.get() && mc.player.getVelocity().y < -maxVelocity.get()) {
            mc.player.setVelocity(mc.player.getVelocity().x, -maxVelocity.get(), mc.player.getVelocity().z);
        }
