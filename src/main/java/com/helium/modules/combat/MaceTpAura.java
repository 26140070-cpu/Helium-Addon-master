...
public class MaceTpAura extends Module {
    ...
    private boolean wasTargetFound = false;

    public MaceTpAura() {
        super(HeliumAddon.CATEGORY, "mace-tp-aura",
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        PlayerInventoryAccessor inventory = (PlayerInventoryAccessor) (Object) mc.player.getInventory();
    ...
    private void sendToGround() {
        ...
        for (int i = 0; i < 256; i++) {
            double testY = groundY - i;
            if (mc.world.getBlockState(new net.minecraft.util.math.BlockPos(
                    (int) Math.floor(x), (int) Math.floor(testY) - 1, (int) Math.floor(z)
        ...
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                x, groundY, z, true, mc.player.horizontalCollision
        ));
        mc.player.setPosition(x, groundY, z);
    }
